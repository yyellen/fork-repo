<template>
  ...
  <section class="md:min-h-screen bg-white px-1 overflow-hidden">
    <div class="Container grid-CMS-3to1 h-hull py-20">
      ...
      <form class="md:col-span-2 flex flex-col" @submit.prevent="createresearch">
        <!-- TODO: config -->
...
import CaseList from '../CMS-UI/List/CaseList.vue'
...
export default {
  ...
  methods: {
    async createresearch() {
      ...
      const addedDoc = await addDoc(researchColRef, this.$data);
      const q = query(researchColRef, orderBy('createdAt'));
    ...
    imageHandler(file, Editor, cursorLocation, resetUploader) {
      const researchRef = ref(fileStorage, `case/innerImg/${file.name}`);
      const uploadTask = uploadBytesResumable(researchRef, file);
      uploadTask.on(
        'state_changed',
        (snapshot) => {
    ...
    redirectCase(id) {
      this.$router.push(`/researchColref/${id}`);
    }
  }
};
</script>