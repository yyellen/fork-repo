<template>
  ...
  <section class="md:h-screen bg-white px-1 overflow-hidden">
    <div class="Container h-full py-20">
      <form class="md:col-span-2 flex flex-col h-full" @submit.prevent="updateNow">
        <!-- TODO: config -->
...
export default {
  ...
  methods: {
    async getNow() {
      ...
      let nwwRef = doc(nowColRef, this.nowId);
      this.docRef = nwwRef;
      let now = await getDoc(this.docRef);
      let nowData = now.data();
      this.title = nowData.title;
      this.subTitle = nowData.subTitle;
      this.user = nowData.user;
      this.today = nowData.today;
      this.paras = nowData.paras;
      this.newPhotoURL = nowData.newPhotoURL;
    ...
    async updateNow() {
      const nowColRef = collection(db, 'new');
      let nowRef = doc(nowColRef, this.nowId);
      this.docRef = nowRef;
  ...
  created() {
    let nowId = this.$route.params.nowId;
    this.nowId = nowId;
    this.getNow();
  },
};
